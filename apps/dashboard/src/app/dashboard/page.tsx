    ...
    <div className="flex h-screen w-full flex-col bg-background">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span className="relative flex h-3 w-3">
              ...
              {status === 'connecting' && <span className="relative inline-flex h-3 w-3 rounded-full bg-yellow-500"></span>}
              {status !== 'connected' && status !== 'connecting' && <span className="relative inline-flex h-3 w-3 rounded-full bg-red-500"></span>}
            </span>
            {status}
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <nav className="w-64 border-r p-4 hidden md:flex flex-col gap-2">
          <Link href="/dashboard" className="flex items-center gap-3 rounded-lg bg-secondary px-3 py-2 text-primary transition-colors">
        ...
        <main className="flex-1 p-6 overflow-hidden flex flex-col gap-4">
          ...
          <div className="flex-1 rounded-xl border bg-card text-card-foreground shadow-sm overflow-hidden flex flex-col">
            <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-sm">
              {messages.length === 0 ? (
                <div className="text-muted-foreground text-center mt-20">Waiting for agent activity...</div>
              ) : (
                messages.map((msg, idx) => (
                  <div key={idx} className="flex flex-col border-b border-border/50 pb-2">
                    <span className="text-xs text-muted-foreground">{new Date(msg.timestamp).toLocaleTimeString()} [{msg.type}]</span>
