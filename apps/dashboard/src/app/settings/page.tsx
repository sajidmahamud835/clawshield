    ...
    <div className="flex h-screen w-full flex-col bg-background">
      ...
      <div className="flex flex-1 overflow-hidden">
        <nav className="w-64 border-r p-4 hidden md:flex flex-col gap-2">
          ...
          <Link href="/settings" className="flex items-center gap-3 rounded-lg bg-secondary px-3 py-2 text-primary transition-colors">
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          <div className="max-w-2xl">
            ...
            <section className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">Keys are encrypted using your hardware JWT and stored in local browser storage via WebCrypto AES-GCM.</p>
              
              <div className="space-y-4 mt-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">Anthropic API Key</label>
                  <input type="password" value={keys.anthropic} onChange={e => setKeys({...keys, anthropic: e.target.value})} className="w-full rounded-md border bg-input px-3 py-2" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">OpenAI API Key (Whisper Fallback)</label>
                  <input type="password" value={keys.openai} onChange={e => setKeys({...keys, openai: e.target.value})} className="w-full rounded-md border bg-input px-3 py-2" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">ElevenLabs API Key</label>
                  <input type="password" value={keys.elevenlabs} onChange={e => setKeys({...keys, elevenlabs: e.target.value})} className="w-full rounded-md border bg-input px-3 py-2" />
                </div>
              </div>

              <div className="pt-4 flex items-center gap-4">
                <button onClick={handleSave} className="rounded-md bg-primary text-primary-foreground px-6 py-2 text-sm font-medium hover:bg-primary/90 transition-colors">
                  Save Changes
                </button>
                {saved && <span className="text-sm text-green-500 font-medium">Saved securely!</span>}
              </div>
            </section>
          </div>
        </main>
